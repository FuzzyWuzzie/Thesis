\chapter{Numerical Characterization of Shear Wave Speed Quantification}
	...
	\label{subsec:shear_results}
		...
		\subsection{Physical Phantom Validation}
			...
			\begin{figure}[!htb]
				...
				\begin{tikzpicture}
					\begin{axis}[
						...
						ymin=0, ymax=3.5,
						draw=black, text=black, fill=black]
							\addplot table {assets/shear/data/shear_experiment_nominal.dat};
							\addplot[mark=none,dashed,ultra thick] plot coordinates {(0, 0) (3.5, 3.5)};
					\end{axis}
				\end{tikzpicture}
			...
			The percent difference of the validation experiments is shown in Fig. \ref{fig:shear_phantom_validation_nominal_mse} where the percent difference was calculated according to equation \ref{equ:percent-error} where $Y_i$ and $\hat{Y}_i$ are the measured and expected values respectively. As expected, the difference between the experimentally-acquired and nominal stiffness ratios for the full range of stiffness ratios studied was relatively small with differences being on the order of \SI{4}{\percent} -- \SI{11}{\percent}.

			\begin{equation}
				\label{equ:percent-error}
				\delta_\percent = \frac{Y_i - \hat{Y}_i}{\hat{Y}_i} \times \SI{100}{\percent}
			\end{equation}

			%Fig. \ref{fig:shear_phantom_validation} shows the relationship between the experimentally acquired lesion stiffness ratios and the simulated lesion stiffness ratios for pairs of parametrically identical lesions. For all but the stiffest of lesions, the 
			\lipsum[1]
			...
			\begin{figure}[!htb]
				...
				\begin{tikzpicture}
					\begin{axis}[
						...
						ylabel={Percent Difference, $\delta_\percent$ (\percent)},
						enlarge x limits=0.25,
						xtick=data,
						symbolic x coords={0.32, 0.56, 1.8, 3.2},
						cycle list name=BarColourPlotCycle]
						\addplot table {assets/shear/data/shear_experiment_nominal_pe.dat};
					\end{axis}
				\end{tikzpicture}
				\caption[]{}
				\label{fig:shear_phantom_validation_nominal_pe}
			\end{figure}
			...
			\begin{figure}[!htb]
				...
				\begin{tikzpicture}
					\begin{axis}[
						...
						ymin=0, ymax=3.5,
						draw=black, text=black, fill=black]
							\addplot table {assets/shear/data/shear_experiment.dat};
							\addplot[mark=none,dashed,ultra thick] plot coordinates {(0, 0) (3.5, 3.5)};
					\end{axis}
				\end{tikzpicture}
				\caption[Experimental validation of shear wave speed quantification model results]{Relation between simulated measured strain ratios and experimental measured strain ratios for a lesion at a depth of \SI{3.5}{\cm} and diameter of \SI{2.0}{\cm} showing general agreement between simulated and experimental cases.}
			...
			\begin{figure}[!htb]
				...
				\begin{tikzpicture}
					\begin{axis}[
						...
						ymajorgrids=true,
						xlabel={Nominal Stiffness Ratio, $E_{nom}$},
						ylabel={Percent Difference, $\delta_\percent$ (\si{\percent})},
						enlarge x limits=0.25,
						xtick=data,
						symbolic x coords={0.32, 0.56, 1.8, 3.2},
						cycle list name=BarColourPlotCycle]
						\addplot table {assets/shear/data/shear_experiment_pe.dat};
					\end{axis}
				\end{tikzpicture}
				\caption[]{}
				\label{fig:shear_phantom_validation_pe}
			\end{figure}
