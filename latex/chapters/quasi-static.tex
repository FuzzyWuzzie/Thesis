\chapter{Numerical Characterization of Quasi-Static Ultrasound Elastography}
	...
	\section{Method}
		...
		\subsection{Finite-Element Model of Tissue Deformation Under Surface Distortion}
			...
			\begin{equation}
				\label{equ:gamma_top}
				\vec{u} = (0, -u_0), \qquad \Gamma = \Gamma_{top}
			\end{equation}
			...
			\begin{figure}[!htb]
				\centering
					\begin{tikzpicture}[x=0.045\textwidth, y=0.045\textwidth, draw=black, text=black, fill=black]
						...
						\draw (0, 13) -- (4, 13);
						\draw (2, 13) node[above] {$\vec{u} = (0, -u_0)$};

						\draw (2, 12.5) node[below]{$\Gamma = \Gamma_{top}$};
						\draw (2, 0) node[above]{$\Gamma = \Gamma_{bottom}$};

					\end{tikzpicture}
				\caption[Quasi-static finite-element model boundary conditions]{Boundary conditions used in the finite-element calculations of soft tissue deformation.}
				\label{fig:qs:boundary_conditions}
			\end{figure}

			From these simulations, displacement fields throughout the domain were calculated which were then used to displace tissue (including scattering centres) in the simulated ultrasound images in both the axial and lateral directions. This process resulted in pairs of pre- and post- compression simulated b-mode images of lesions of varying parameters which could then be analyzed and characterized. Sample source code for calculating these finite-element models using COMSOL Multiphysics\textsuperscript{\textregistered}\  is given in listing \ref{lst:compression} in Appendix \ref{app:source_code}.
	...
	\section{Results and Discussion}
		...
		\subsection{Finite Element Models of Ultrasound and Deformation}
			...
			\begin{figure*}[!htb]
				...
				\subfloat[]{
					\begin{tikzpicture}
						\begin{axis}[width=0.65\textwidth, enlargelimits=false, unit vector ratio*=1 1 1, axis on top, xlabel={Lateral deviation, $x$ (\si{\cm})}, ylabel={Depth, $d$ (\si{\cm})}, y dir=reverse, colormap name={RdBu}, colorbar, point meta min=-1.17, point meta max=1.17, colorbar style={at={(1.05,0)}, anchor=south west, width=0.03\textwidth, ylabel={Lateral Displacement (\si{\mm})},
						draw=black, text=black, fill=black},
						draw=black, text=black, fill=black]
							\addplot graphics[xmin=-2,xmax=2,ymin=0,ymax=12.5]{assets/quasistatic/images/displacement_u_colour.png};
							\draw[->,draw=black] (axis cs:0,7) -- (axis cs:0,8.75);
							\draw (axis cs:0,7) node[above]{Lesion};
						\end{axis}
					\end{tikzpicture}
				...
				\subfloat[]{
					\begin{tikzpicture}
						\begin{axis}[width=0.65\textwidth, enlargelimits=false, unit vector ratio*=1 1 1, axis on top, xlabel={Lateral deviation, $x$ (\si{\cm})}, ylabel={Depth, $d$ (\si{\cm})}, y dir=reverse, colormap name={YlOrRd}, colorbar, point meta min=-6.25, point meta max=0, colorbar style={at={(1.05,0)}, anchor=south west, width=0.03\textwidth, ylabel={Axial Displacement (\si{\mm})},
						...
						draw=black, text=black, fill=black]
							...
							\draw[->,draw=white] (axis cs:0,7) -- (axis cs:0,8.75);
							\draw (axis cs:0,7) node[above]{\color{white}Lesion};
						\end{axis}
					\end{tikzpicture}
				...
				\subfloat[]{
					\begin{tikzpicture}
						...
						draw=black, text=black, fill=black]
							...
							\draw[->,draw=white] (axis cs:0,8) -- (axis cs:0,10);
							\draw (axis cs:0, 8) node[above]{\begin{tabular}{c} \color{white}\footnotesize Lesion \\ \color{white}\footnotesize undetectable\end{tabular}};
						\end{axis}
					\end{tikzpicture}
				...
				\subfloat[]{
					\begin{tikzpicture}
						...
						draw=black, text=black, fill=black]
							...
							\draw[->,draw=white] (axis cs:0,10) -- (axis cs:0,12);
							\draw (axis cs:0,10) node[above]{\begin{tabular}{c} \color{white}\footnotesize Spurious \\ \color{white}\footnotesize data\end{tabular}};
						\end{axis}
					\end{tikzpicture}
		...
		\subsection{Resulting Elastograms}
			...
			\begin{figure}[!htb]
				\centering
					\begin{tikzpicture}
						\begin{axis}[width=\columnwidth, enlargelimits=false, unit vector ratio*=1 1 1, axis on top, xlabel={Lateral deviation, $x$ (\si{\cm})}, ylabel={Depth, $d$ (\si{\cm})}, y dir=reverse, colormap name={YlOrRd}, colorbar, point meta min=0, point meta max=7, colorbar style={yticklabel={\pgfmathprintnumber{\tick}\,\percent},at={(1.05,0)}, width=0.03\textwidth, ylabel={Compressive Strain}, anchor=south west,
						draw=black, text=black, fill=black},
						draw=black, text=black, fill=black]
							\addplot graphics[xmin=-2,xmax=2,ymin=0,ymax=12.5]{assets/quasistatic/elastograms/e040_colour.png};
							\draw[->] (axis cs:0,7.5) -- (axis cs:0,9.5);
							\draw (axis cs:0,7.5) node[above]{Stiff lesion};
							
							\draw[->] (axis cs:0,3) -- (axis cs:0, 1);
							\draw (axis cs:0,3) node[below]{\begin{tabular}{c} \footnotesize Transducer \\ \footnotesize shadow \end{tabular}};

							\draw (axis cs:0,12) node[above]{\footnotesize \color{white} Spurious data};
							\draw[->,draw=white] (axis cs:0, 11.75) -- (axis cs:0,12.25);
						\end{axis}
					\end{tikzpicture}
		...
		\subsection{Numerical Characterizations}
			...
			\begin{equation}
				\label{equ:qs_percent_error}
				\varepsilon_p = \left|\frac{Y_i - \hat{Y}_i}{\hat{Y}_i}\right| \times \SI{100}{\percent}
			\end{equation}
			...
			\begin{figure}[!htb]
				...
				\begin{tikzpicture}
					\begin{axis}[
						scale only axis,
						height=3in,
						width=\textwidth-\widthof{100}-0.5in,
						xlabel={Normalized Characterization Index, $NCI$},
						ylabel={Percent Error, $\varepsilon_p$ (\si{\percent})},
						grid=major,
						legend style={at={(axis cs:0.55,69)},anchor=north,font=\small,nodes={right}},
						clip=true,
						cycle list name=ColourPlotRegressionCycle,
						ymin=20, ymax=70,
						draw=black, text=black, fill=black]
						\addplot+[forget plot] table {assets/quasistatic/data/normalized_size.dat}; \pgfplotsset{cycle list shift=1}
						\addplot+[forget plot] table[y={create col/linear regression={y=perror}}] {assets/quasistatic/data/normalized_size.dat}; \pgfplotsset{cycle list shift=2}
						\xdef\slopeA{\pgfplotstableregressiona} \xdef\interA{\pgfplotstableregressionb}
						\addplot+[forget plot] table {assets/quasistatic/data/normalized_depth.dat}; \pgfplotsset{cycle list shift=3}
						\addplot+[forget plot] table[y={create col/linear regression={y=perror}}] {assets/quasistatic/data/normalized_depth.dat}; \pgfplotsset{cycle list shift=4}
						\xdef\slopeB{\pgfplotstableregressiona} \xdef\interB{\pgfplotstableregressionb}
						\addplot+[forget plot] table {assets/quasistatic/data/normalized_bottomsep.dat}; \pgfplotsset{cycle list shift=5}
						\addplot+[forget plot] table[y={create col/linear regression={y=perror}}] {assets/quasistatic/data/normalized_bottomsep.dat}; \pgfplotsset{cycle list shift=6}
						\xdef\slopeC{\pgfplotstableregressiona} \xdef\interC{\pgfplotstableregressionb}
						\addplot+[forget plot] table {assets/quasistatic/data/normalized_frequency.dat}; \pgfplotsset{cycle list shift=7}
						\addplot+[forget plot] table[y={create col/linear regression={y=perror}}] {assets/quasistatic/data/normalized_frequency.dat}; \pgfplotsset{cycle list shift=8}
						\xdef\slopeD{\pgfplotstableregressiona} \xdef\interD{\pgfplotstableregressionb}
						\addplot+[forget plot] table {assets/quasistatic/data/normalized_strain.dat}; \pgfplotsset{cycle list shift=9}
						\addplot+[forget plot] table[y={create col/linear regression={y=perror}}] {assets/quasistatic/data/normalized_strain.dat}; \pgfplotsset{cycle list shift=10}
			...
			\begin{figure}[!htb]
				...
				\begin{tikzpicture}
					\begin{axis}[
						...
						cycle list name=ColourPlotRegressionCycle,
						draw=black, text=black, fill=black]
						\addplot+[forget plot] table {assets/quasistatic/data/normalized_lesion_sep.dat}; \pgfplotsset{cycle list shift=1}
						\addplot+[forget plot] table[y={create col/linear regression={y=perror}}] {assets/quasistatic/data/normalized_lesion_sep.dat}; \pgfplotsset{cycle list shift=2}
						\xdef\slopeA{\pgfplotstableregressiona} \xdef\interA{\pgfplotstableregressionb}
						
						\addplot+[forget plot] table {assets/quasistatic/data/normalized_blur_radius.dat}; \pgfplotsset{cycle list shift=3}
						\addplot+[forget plot] table[y={create col/linear regression={y=perror}}] {assets/quasistatic/data/normalized_blur_radius.dat}; \pgfplotsset{cycle list shift=4}
						\xdef\slopeB{\pgfplotstableregressiona} \xdef\interB{\pgfplotstableregressionb}
						
						\addplot+[forget plot] table {assets/quasistatic/data/normalized_blob_density.dat}; \pgfplotsset{cycle list shift=5}
						\addplot+[forget plot] table[y={create col/linear regression={y=perror}}] {assets/quasistatic/data/normalized_blob_density.dat}; \pgfplotsset{cycle list shift=6}
						\xdef\slopeC{\pgfplotstableregressiona} \xdef\interC{\pgfplotstableregressionb}
						
						\addplot+[forget plot] table {assets/quasistatic/data/normalized_blob_radius.dat}; \pgfplotsset{cycle list shift=7}
						\addplot+[forget plot] table[y={create col/linear regression={y=perror}}] {assets/quasistatic/data/normalized_blob_radius.dat}; \pgfplotsset{cycle list shift=8}
						\xdef\slopeD{\pgfplotstableregressiona} \xdef\interD{\pgfplotstableregressionb}
						
						\addplot+[forget plot] table {assets/quasistatic/data/normalized_human_size.dat}; \pgfplotsset{cycle list shift=9}
						\addplot+[forget plot] table[y={create col/linear regression={y=perror}}] {assets/quasistatic/data/normalized_human_size.dat}; \pgfplotsset{cycle list shift=10}
						\xdef\slopeE{\pgfplotstableregressiona} \xdef\interE{\pgfplotstableregressionb}
						
						\addplot+[forget plot] table {assets/quasistatic/data/normalized_human_depth.dat}; \pgfplotsset{cycle list shift=11}
						\addplot+[forget plot] table[y={create col/linear regression={y=perror}}] {assets/quasistatic/data/normalized_human_depth.dat}; \pgfplotsset{cycle list shift=12}
			...
			\begin{figure}[!htb]
				...
				\characterizationPic{%
					...
					\characterizationLegend{Lesion Depth}{d}%
						...
						{\SI{8.5}{\cm}}%
						{\SI{10.0}{\cm}}%
				}{spherical_depth}{3.125,0.5}{north west}{ymin=0.5}
			...
			\begin{figure}[!htb]
				...
				\characterizationPic{%
					\characterizationPlots{strainRatio}%
						{quasistatic/data/circular_bottomsep_000.dat}%
						{quasistatic/data/circular_bottomsep_125.dat}%
						{quasistatic/data/circular_bottomsep_250.dat}%
				}{%
					\characterizationLegend{Lesion Altitude}{h}%
						{\SI{1.25}{\cm}}%
						{\SI{2.50}{\cm}}%
						{\SI{3.75}{\cm}}%
				}{spherical_altitude}{3.125,0.5}{north west}{ymin=0.5}
			...
			\begin{figure}[!htb]
				...
				\characterizationPic{%
					...
					\characterizationLegend{Probing Frequency}{f}%
						...
						{\SI{4}{\MHz}}%
						{\SI{8}{\MHz}}%
				}{spherical_frequency}{3.125,0.5}{north west}{ymin=0.5}
			...
			\begin{figure}[!htb]
				...
				\characterizationPic{%
					...
					\characterizationLegend{Applied Strain}{\varepsilon_{app}}%
						...
						{\SI{5.0}{\percent}}%
						{\SI{10.0}{\percent}}%
				}{spherical_strain}{3.125,0.5}{north west}{ymin=0.5}
			...
			\begin{figure}[!htb]
				...
				\characterizationPic{%
					\characterizationPlots{strainRatio}%
						{quasistatic/data/separation_0125.dat}%
						{quasistatic/data/separation_0150.dat}%
						{quasistatic/data/separation_0175.dat}%
						{quasistatic/data/separation_0200.dat}%
				}{%
					\characterizationLegend{Lesion Separation Distance}{\delta_{sep}}%
						{\SI{1.25}{\cm}}%
						{\SI{1.50}{\cm}}%
						{\SI{1.75}{\cm}}%
						{\SI{2.00}{\cm}}%
				}{colocated_separation}{3.125,0.5}{north west}{ymin=0.5,ymax=2.25}
			...
			\begin{figure}[!htb]
				...
				\begin{tikzpicture}
					\begin{axis}[
						...
						point meta min=0,
						point meta max=7,
						colorbar style={yticklabel={\pgfmathprintnumber{\tick}\,\si{\percent}}, at={(1.05,0)}, width=0.03\textwidth, ylabel={Compressive Strain}, anchor=south west,
						draw=black, text=black, fill=black},
						draw=black, text=black, fill=black]
							\addplot graphics[xmin=-2,xmax=2,ymin=0,ymax=12.5]{assets/quasistatic/elastograms/e080_colour.png};
							\draw[<->, draw=black] (axis cs:-0.75,10.5) -- (axis cs:0,11.5) -- (axis cs:0.75,10.5);
							\draw(axis cs:0, 11.5) node[below]{\color{black}\scriptsize Stiff lesions};
							\draw[<-, draw=black] (axis cs:0,9) -- (axis cs:0, 8);
							\draw(axis cs:0, 8) node[above]{\begin{tabular}{c} \color{black}\scriptsize Apparent stress \\ \color{black}\scriptsize concentration \end{tabular}};
					\end{axis}
				\end{tikzpicture}
			...
			\begin{figure}[!htb]
				...
				\characterizationPic{%
					\characterizationPlots{strainRatio}%
						{quasistatic/data/blur_radius_01.dat}%
						{quasistatic/data/blur_radius_25.dat}%
						{quasistatic/data/blur_radius_50.dat}%
						{quasistatic/data/blur_radius_75.dat}%
				}{%
					\characterizationLegend{Blur Radius}{b_r}%
						...
						{\SI{5.0}{\mm}}%
						{\SI{7.5}{\mm}}%
				}{blur_radius}{3.125,0.5}{north west}{ymin=0.5}
			...
			\begin{figure}[!htb]
				...
				\characterizationPic{%
					\characterizationPlots{strainRatio}%
						{quasistatic/data/blob_density_10.dat}%
						{quasistatic/data/blob_density_20.dat}%
						{quasistatic/data/blob_density_30.dat}%
						{quasistatic/data/blob_density_40.dat}%
				}{%
				...
				}{cluster_density}{3.125,0.5}{north west}{ymin=0.5}
					...
					\begin{tikzpicture}
						\begin{axis}[
							...
							draw=black, text=black, fill=black]
								...
								\draw[<-] (axis cs:0,10) -- (axis cs:0, 8);
								\draw (axis cs:0,8) node[above]{\begin{tabular}{c} \scriptsize Individual lesions \\ \scriptsize undetectable \end{tabular}};
								%\draw (axis cs:0,8) node[above]{\begin{\tabular}{c}\footnotesize Individual lesions \\ \footnotesize undetectable\end{tabular}};
						\end{axis}
					\end{tikzpicture}
					\label{fig:blob_elastogram_b}
				}
				\caption[Sample elastogram for a set of clustered lesions]{Stiffness map \protect\subref{fig:blob_elastogram_a} and corresponding elastogram \protect\subref{fig:blob_elastogram_b} for a group a small lesions with a density of 10 lesions per \si{\cm\squared} grouped in a \SI{2.5}{\cm} diameter circle at a depth of \SI{10}{\cm} interrogated with a \SI{4}{\MHz} probing frequency and \SI{5}{\percent} applied strain. In \protect\subref{fig:blob_elastogram_a}, white regions are regular tissue while black regions are the small lesions. In the elastogram, individual lesions do not stand out, rather the entire region of lesions appears as one large region of unhealthy tissue.}
			...
			\begin{figure}[!htb]
				...
				\characterizationPic{%
					\characterizationPlots{strainRatio}%
						{quasistatic/data/blob_radius_05.dat}%
						{quasistatic/data/blob_radius_10.dat}%
						{quasistatic/data/blob_radius_15.dat}%
				}{%
			...
			\begin{figure}[!htb]
				...
				\characterizationPic{%
					\characterizationPlots{strainRatio}%
						{quasistatic/data/human_size_05.dat}%
						{quasistatic/data/human_size_10.dat}%
						{quasistatic/data/human_size_20.dat}%
						{quasistatic/data/human_size_25.dat}%
				}{%
					\characterizationLegend{Lesion Width}{\diameter L}%
						...
						{\SI{2.0}{\cm}}%
						{\SI{2.5}{\cm}}%
				}{human_width}{3.125,0.6}{north west}{ymax=2}
				...
				\begin{tikzpicture}
					\begin{axis}[
						...
						draw=black, text=black, fill=black]
							...
							\draw[<-,thick] (axis cs:0,7.25) -- (axis cs:0,5.5);
							\draw (axis cs:0,5.5) node[above]{\footnotesize Lesion undetectable};
					\end{axis}
				\end{tikzpicture}
			...
			\begin{figure}[!htb]
				...
				\characterizationPic{%
					\characterizationPlots{strainRatio}%
						{quasistatic/data/human_bottom_sep_625.dat}%
						{quasistatic/data/human_bottom_sep_675.dat}%
						{quasistatic/data/human_bottom_sep_725.dat}%
				}{%
					\characterizationLegend{Lesion Depth}{d}%
						{\SI{6.25}{\cm}}%
						{\SI{6.75}{\cm}}%
						{\SI{7.25}{\cm}}%
				}{human_depth}{3.125,0.6}
				...
				\begin{tikzpicture}
					\begin{axis}[
						...
						draw=black, text=black, fill=black]
							...
							\node [anchor=north west](c) at (axis cs:0.55,2.45) {\includegraphics{assets/insets/spherical.pdf}};
					\end{axis}
				\end{tikzpicture}
