...
\begin{document}
	\begin{tikzpicture}[x=0.0507\textwidth, y=0.0507\textwidth]
		...
		% the lesion
		\draw[fill=lesionColour] (1, 1) circle(0.5);

		% the text
		%\draw[domain=0:1,samples=500,smooth,variable=\x,black] plot ({\x+0.3},{cos(320*pi*\x+180)*gauss(0.5,0.25,\x)*0.25+2.5});
		\draw[domain=0:1,smooth,variable=\y,black] plot ({cos(320*pi*\y+180)*gauss(0.5,0.25,\y)*0.25+0.75},{\y+1.75});
		\draw (1.55,2.25) node{$f$};
	\end{tikzpicture}
\end{document}